...
* (已修复) ~~对话框弹出时，页面锁定滚动我还没有找到合适的方法能保证页面不会自动滑回顶端且内容不发生偏移。我目前使用的办法是记录下滚动的位置，对话框弹出时使用 CSS 属性 `transform` 对整个文档向上平移，将文档的 `overflow` 属性设为 `hidden`，关闭后再恢复 `overflow` 属性、取消平移，但是这样带来的问题是对话框弹出后页面会有明显的闪动且背景图发生错位。~~
